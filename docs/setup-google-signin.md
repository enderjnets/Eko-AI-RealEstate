...
- **`AUTH_SECRET` is required** once `AUTH_ENABLED=true`, and the app refuses to sign tokens without it. It used to fall back to a value derived from `DASHBOARD_PASSWORD`, which meant the person who answers the office phone could derive the key and mint themselves a token claiming any organization and platform-operator rights. `openssl rand -hex 32`.
