...
HANDLED = {
    ...
    # The only bounded columns are `title` and `recipient`, both typed by an
    # operator at `scripts/create_brief.py` — refusing a 300-character title at
    # the terminal, with nothing else in the transaction, is the right answer.
    #
    # The half worth saying out loud is what is NOT here: the public endpoint
    # writes `answers` (JSONB, bounded by BRIEF_MAX_ANSWERS) and two
    # timestamps, and touches no bounded column at all. Nothing a partner types
...
class TestEveryUniqueStringKeyHasBeenThoughtAbout:
    ...
    DECIDED = {
        ...
        # Generated by us and never derived from input: `token_urlsafe(32)` is
        # 43 characters of base64url into a column of 64. It is also the only
        # credential the brief has, so it must be stored exactly — a truncated
