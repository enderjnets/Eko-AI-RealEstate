...
export const CHANGELOG: VersionEntry[] = [
  ...
  },
  {
    version: "0.143.9",
    date: "2026-09-24",
    title: { en: "Denver shorts long enough to make their point", es: "Shorts de Denver con tiempo para explicarse" },
    changes: [{
      en: "Denver, Decoded, Weekend and Market videos now run about 20–30 seconds instead of 8–18, with more scenes. At the old length a video ended before it said what it was about. The sales videos are unchanged.",
      es: "Los vídeos de Denver, Decoded, Weekend y Market duran ahora unos 20–30 segundos en vez de 8–18, con más escenas. Con la duración anterior el vídeo terminaba antes de decir de qué trataba. Los vídeos de venta no cambian.",
    }],
  },
  {
    version: "0.143.8",
  ...
  },
  {
    version: "0.143.3",
    date: "2026-09-23",
    title: { en: "The render card says what the engine is doing", es: "La tarjeta del render dice qué hace el motor" },
    changes: [{
      en: "While the video engine works, the card now reads “Building the video — no progress shows until it ends” instead of a raw code. That step stays at 5% for several minutes on purpose. A step the console does not know yet shows the generic line instead of its code.",
      es: "Mientras el motor de vídeo trabaja, la tarjeta dice ahora «Montando el vídeo — no marca avance hasta terminar» en lugar de un código. Ese paso se queda en el 5 % varios minutos a propósito. Un paso que el panel aún no conoce muestra el texto genérico en lugar de su código.",
    }],
  },
  {
    version: "0.143.2",
  ...
  },
  {
    version: "0.143.0",
    date: "2026-09-23",
    title: { en: "New content lines fill the free days", es: "Las líneas nuevas ocupan los días libres" },
    changes: [{
  ...
  },
  {
    ...
    changes: [
      ...
      { en: "The captions speak only for what the card holds: the server returns the twenty newest videos and the router does not raise it, so a thirty-day range with more than twenty arrives already cut. A total labelled \"in range\" would be a number with the wrong name. The cut predates this release and is still there; what changes is that nothing claims otherwise.", es: "Los rótulos solo hablan de lo que la tarjeta tiene: el servidor devuelve los veinte vídeos más recientes y el router no lo sube, así que un rango de treinta días con más de veinte llega ya recortado. Un total rotulado «del rango» sería un número con el nombre equivocado. El recorte es anterior a esta release y sigue ahí; lo que cambia es que ya nada afirma lo contrario." },
  ...
  },
  {
    ...
    date: "2026-09-17",
    title: {
      en: "A rejection with a reason becomes a work order",
      es: "Un rechazo con motivo se convierte en una orden de trabajo",
    },
    changes: [
      { en: "Rejecting a piece used to write one sentence into a column nothing ever read. Four pieces were rejected for the same missing call to action over three days — 66, 70, 71 and 73, \"There is not call to action at the end\", \"NO esta cerrando con un CTA\", \"CTA missing\" — and the fifth came out with it missing too. Every rejection is now recorded with a snapshot of exactly what was rejected, read back by a sweep that runs before any new draft is written, and closed with what was done about it.", es: "Rechazar una pieza escrib\u00eda una frase en una columna que nadie le\u00eda nunca. Cuatro piezas se rechazaron por la misma llamada a la acci\u00f3n ausente en tres d\u00edas — 66, 70, 71 y 73, \"There is not call to action at the end\", \"NO esta cerrando con un CTA\", \"CTA missing\" — y la quinta sali\u00f3 igual. Ahora cada rechazo se registra con una instant\u00e1nea de lo que se rechaz\u00f3, lo relee un barrido que corre antes de escribir ning\u00fan borrador nuevo, y se cierra diciendo qu\u00e9 se hizo." },
      { en: "The sweep takes the cheapest action that could answer the complaint. If the narration already says the address, the text was right and only the video was old: it is made again, and no model is asked. If the narration lost its sign-off, the sign-off is put back first. If the problem is the words — a figure, the language, Fair Housing, or a reason nobody could place — the model is asked to correct its own draft rather than write a new one, so everything the reviewer did not object to survives. A filmed clip, or a calculated piece whose inputs cannot be recovered, goes to a person and says so.", es: "El barrido toma la acci\u00f3n m\u00e1s barata que pueda responder a la queja. Si la narraci\u00f3n ya dice la direcci\u00f3n, el texto estaba bien y solo el v\u00eddeo era viejo: se vuelve a montar y no se consulta a ning\u00fan modelo. Si la narraci\u00f3n perdi\u00f3 su despedida, se le devuelve primero. Si el problema son las palabras — una cifra, el idioma, Fair Housing, o un motivo que nadie pudo colocar — se le pide al modelo que corrija su propio borrador en vez de escribir otro, para que sobreviva todo aquello a lo que el revisor no puso pegas. Un clip filmado, o una pieza calculada cuyos datos no se pueden recuperar, va a una persona y lo dice." },
      { en: "It has ceilings, and they are small because every lap is a real charge: one automatic correction per rejection, two per piece for its whole life, three per agency per day. Past that it stops, leaves the piece rejected, and emails the operator with the reason and everything already tried. The piece that proved the ceiling was needed is 70, where \"no tiene CTA\" was the tail of a sentence whose real complaint was the opening.", es: "Tiene topes, y son peque\u00f1os porque cada vuelta es un cargo real: una correcci\u00f3n autom\u00e1tica por rechazo, dos por pieza en toda su vida, tres por agencia y d\u00eda. Pasado eso para, deja la pieza rechazada y escribe al operador con el motivo y todo lo ya intentado. La pieza que demostr\u00f3 que el tope hac\u00eda falta es la 70, donde \"no tiene CTA\" era la cola de una frase cuya queja real era el arranque." },
  ...
  },
  {
    ...
    date: "2026-09-16",
    title: {
      en: "The visit that came from a machine room, and the neighbour who did not",
      es: "La visita que vino de una sala de m\u00e1quinas, y el vecino que no",
    },
    changes: [
      { en: "A second traffic classifier, for the visits that come from a machine room. Seven of the eight sessions that ever started the contact form were infrastructure: Clonee, Boardman, Prineville, Forest City, Lule\u00e5 and Springfield, 38 to 41 events each and zero scroll, plus Council Bluffs and Ashburn arriving with one event and no referrer. The eighth was our own Playwright and says so in its traffic_class, which is the trap: querying form_started_at without reading traffic_class produced a confident report of a real person, and that mistake was made and published here before being caught. No real visitor has ever started the form. The rule needs a data-centre city AND zero scroll, and it only ever writes over unknown.", es: "Un segundo clasificador de tr\u00e1fico, para las visitas que vienen de una sala de m\u00e1quinas. Siete de las ocho sesiones que han empezado alguna vez el formulario de contacto eran infraestructura: Clonee, Boardman, Prineville, Forest City, Lule\u00e5 y Springfield, con 38 a 41 eventos cada una y cero scroll, m\u00e1s Council Bluffs y Ashburn llegando con un solo evento y sin referente. La octava era nuestro propio Playwright y lo dice en su traffic_class, que es la trampa: consultar form_started_at sin leer traffic_class produjo un informe confiado sobre una persona real, y ese error se cometi\u00f3 y se public\u00f3 aqu\u00ed antes de detectarse. Nadie de fuera ha empezado nunca el formulario. La regla exige ciudad de centro de datos Y cero scroll, y solo escribe encima de unknown." },
      { en: "The list of cities is deliberately short and keeps evidence apart from expectation. Dublin, San Jose, Chicago, New York and Mountain View are NOT on it: all host enormous cloud capacity and all are places hundreds of thousands of people live, some of whom move to Denver. Boydton is left off although it is a Microsoft region, because its four visits here scrolled to 100%. The asymmetry decides every one of these calls: with zero leads, a false automated throws away the only kind of row that matters, while a false unknown only makes a denominator noisier.", es: "La lista de ciudades es deliberadamente corta y separa la evidencia de la expectativa. Dubl\u00edn, San Jos\u00e9, Chicago, Nueva York y Mountain View NO est\u00e1n: todas alojan cantidades enormes de nube y todas son sitios donde viven cientos de miles de personas, algunas de las cuales se mudan a Denver. Boydton se queda fuera aunque sea una regi\u00f3n de Microsoft, porque sus cuatro visitas aqu\u00ed llegaron al 100% de scroll. La asimetr\u00eda decide todos estos casos: con cero leads, un automated falso tira la \u00fanica clase de fila que importa, mientras que un unknown falso solo ensucia un denominador." },
      { en: "Lule\u00e5 is stored in our own database as the mojibake Lule\u00c3\u00a5 — the bytes are 4c756c65c383c2a5, which is the \u00e5 read as Latin-1 and written back as UTF-8. That is a real defect in how the Cloudflare city header is decoded and it affects every non-ASCII city name. Both spellings are matched on purpose: only the correct one would give a list that looks right and catches nothing, and only the broken one would stop working the day the header is fixed.", es: "Lule\u00e5 est\u00e1 guardada en nuestra propia base como el mojibake Lule\u00c3\u00a5 — los bytes son 4c756c65c383c2a5, que es la \u00e5 le\u00edda como Latin-1 y reescrita como UTF-8. Es un defecto real en c\u00f3mo se decodifica la cabecera de ciudad de Cloudflare y afecta a toda ciudad con nombre no ASCII. Se emparejan las dos graf\u00edas a prop\u00f3sito: solo la correcta dar\u00eda una lista que parece bien y no captura nada, y solo la rota dejar\u00eda de funcionar el d\u00eda que se arregle la cabecera." },
      { en: "One rule that was asked for was measured and then NOT implemented: filing any one-event, zero-scroll session as automated. Of the sessions it would have caught, about thirty-six are Denver-area — Aurora, Denver, Parker, The Pinery, Wheat Ridge — including eight that arrived inside the Facebook app from the 11-sep share, the best day this site has had. Those are people who landed and left without scrolling, which until v0.108.0 was the reasonable response to a calculator that opened on two empty fields. The metric that matters already excludes them by requiring 50% scroll, so the rule would have cost the diagnosis and bought nothing.", es: "Una regla que se pidi\u00f3 se midi\u00f3 y NO se implement\u00f3: marcar como automated cualquier sesi\u00f3n de un evento y cero scroll. De las sesiones que habr\u00eda capturado, unas treinta y seis son del \u00e1rea de Denver — Aurora, Denver, Parker, The Pinery, Wheat Ridge — incluidas ocho que llegaron dentro de la app de Facebook desde el compartido del 11-sep, el mejor d\u00eda que ha tenido esta p\u00e1gina. Son personas que entraron y se fueron sin deslizar, que hasta v0.108.0 era la respuesta razonable a una calculadora que abr\u00eda con dos campos vac\u00edos. La m\u00e9trica que importa ya las excluye al exigir 50% de scroll, as\u00ed que la regla habr\u00eda costado el diagn\u00f3stico sin comprar nada." },
    ],
  },
  {
    version: "0.108.0",
    date: "2026-09-16",
    title: {
      en: "The number the video promised, on the page it lands on",
      es: "El n\u00famero que promet\u00eda el v\u00eddeo, en la p\u00e1gina donde aterriza",
    },
    changes: [
      { en: "The videos promise a number and the page asked for it back. Eleven Shorts point at /calculator and five of them say \"$2,600 a month, $40,000 saved\" out loud, but both money fields started empty, so the figure only appeared once the visitor typed. YouTube had sent 28 sessions and 23 of them were a single event with 0% scroll: promised an answer, met a form. /calculator now reads rent, savings and credit from the link, using the same rules its own fields use, and opens on the answer. Verified end to end against the real caption: ?rent=2600&savings=40000 shows $343,000, the exact figure the caption names. A link with no parameters behaves exactly as before, and a link carrying only rent seeds only rent — a savings figure is never invented, because an untouched savings field means \"not yet\" and zero would produce a floor price nobody asked for.", es: "Los v\u00eddeos prometen un n\u00famero y la p\u00e1gina lo volv\u00eda a pedir. Once Shorts apuntan a /calculator y cinco dicen en voz alta «$2.600 al mes, $40.000 ahorrados», pero los dos campos de dinero empezaban vac\u00edos, as\u00ed que la cifra solo aparec\u00eda si el visitante escrib\u00eda. YouTube hab\u00eda mandado 28 sesiones y 23 fueron de un solo evento y 0% de scroll: les prometieron una respuesta y encontraron un formulario. Ahora /calculator lee rent, savings y credit del enlace, con las mismas reglas que usan sus propios campos, y abre directamente en la respuesta. Verificado de punta a punta contra la caption real: ?rent=2600&savings=40000 muestra $343.000, exactamente la cifra que nombra la caption. Un enlace sin par\u00e1metros se comporta igual que antes, y uno que solo lleva rent rellena solo rent — nunca se inventa una cifra de ahorros, porque un campo de ahorros sin tocar significa «todav\u00eda no» y un cero dar\u00eda un precio suelo que nadie pidi\u00f3." },
      { en: "The fix that brings the answer onto the screen was doing nothing in a browser with smooth scrolling switched off. Measured on production on 16-sep, Chrome 152 on macOS with prefers-reduced-motion reporting false: scrollIntoView({behavior:\"smooth\"}) left scrollY at 0, while the same call with \"auto\" moved it 954px. In that browser the whole of v0.102.8 was inert — the figure rendered 271px below the bottom of the window and the page never moved, which is the exact failure v0.102.8 existed to end. It is not most visitors, but it is everyone who has turned the setting off, and it is the browser this gets verified in, which is how a working fix gets reported broken. The page now asks for the smooth scroll and then checks, 150ms later, whether anything actually moved; if nothing did, it jumps. Any movement counts as the person having taken over.", es: "El arreglo que trae la respuesta a la pantalla no hac\u00eda nada en un navegador con el desplazamiento suave desactivado. Medido en producci\u00f3n el 16-sep, Chrome 152 en macOS y con prefers-reduced-motion en falso: scrollIntoView({behavior:\"smooth\"}) dejaba scrollY en 0, mientras la misma llamada con \"auto\" lo mov\u00eda 954px. En ese navegador todo v0.102.8 era inerte — la cifra se dibujaba 271px por debajo del borde de la ventana y la p\u00e1gina no se mov\u00eda, que es exactamente el fallo que v0.102.8 exist\u00eda para terminar. No es la mayor\u00eda de visitantes, pero s\u00ed todo el que tenga la opci\u00f3n apagada, y es el navegador desde el que esto se verifica, que es como un arreglo que funciona se reporta roto. Ahora la p\u00e1gina pide el desplazamiento suave y comprueba, 150ms despu\u00e9s, si algo se movi\u00f3 de verdad; si no, salta. Cualquier movimiento cuenta como que la persona ha tomado el control." },
      { en: "Three short links so a share can be thanked. On 11-sep somebody posted the site on Facebook and it brought 26 real sessions from Denver suburbs, the best day the site has had — and nobody knows who, so it cannot be repeated or asked for again. /n, /r and /e now carry utm_source=partner and name the person who shared, the same way /tt and /ig name the network. The tag is the first name rather than the initial, because the point is that somebody reads the breakdown weeks later without a decoder ring.", es: "Tres enlaces cortos para poder agradecer a quien comparte. El 11-sep alguien public\u00f3 el sitio en Facebook y trajo 26 sesiones reales de los suburbios de Denver, el mejor d\u00eda que ha tenido la p\u00e1gina — y nadie sabe qui\u00e9n fue, as\u00ed que no se puede repetir ni volver a pedir. /n, /r y /e llevan ahora utm_source=partner y nombran a quien comparti\u00f3, igual que /tt e /ig nombran a la red. La etiqueta es el nombre de pila y no la inicial, porque el objetivo es que alguien lea el desglose semanas despu\u00e9s sin necesitar una tabla de equivalencias." },
      { en: "/sitemap.xml answered 404, so the page the whole funnel points at was not in Google's index. It now lists the five public pages as absolute URLs on the brand domain, derived from PUBLIC_PATHS — the same constant the middleware uses — rather than a hand-kept second list, which is how a /brief/<token> URL ends up published to every crawler. A robots.txt naming the sitemap comes with it; whether Cloudflare's own robots.txt lets that line through has to be read from the served file after deploying. With no brand domain configured both produce nothing, the rule lib/hosts.ts sets for every value in it: unset means do nothing, never guess.", es: "/sitemap.xml daba 404, as\u00ed que la p\u00e1gina a la que apunta todo el embudo no estaba en el \u00edndice de Google. Ahora lista las cinco p\u00e1ginas p\u00fablicas como URLs absolutas del dominio de marca, derivadas de PUBLIC_PATHS — la misma constante que usa el middleware — y no de una segunda lista escrita a mano, que es como una URL /brief/<token> acaba publicada a todos los rastreadores. Viene con un robots.txt que nombra el sitemap; si Cloudflare deja pasar esa l\u00ednea hay que leerlo del fichero servido tras desplegar, nunca del fuente. Sin dominio de marca configurado los dos no producen nada, la norma que lib/hosts.ts fija para todos sus valores: sin configurar significa no hacer nada, nunca adivinar." },
    ],
  },
  {
    version: "0.107.2",
  ...
  },
  {
    ...
    changes: [
      ...
      { en: "Also inside this version, written by another session and left without a number of its own: the note that goes out when a partner answers their brief used to print the machine\u2019s own words — “p1: state: out” — where it now prints the person\u2019s name and what was decided about them.", es: "Tambien dentro de esta version, escrito por otra sesion y dejado sin numero propio: el aviso que sale cuando un socio contesta su hoja imprimia las palabras de la maquina — «p1: state: out» — donde ahora imprime el nombre de la persona y que se decidio sobre ella." },
  ...
  },
  {
    ...
    date: "2026-09-06",
    title: {
      en: "The fall guide leads with the season, and shows the places",
      es: "La guía de otoño abre con la temporada, y enseña los sitios",
    },
    changes: [
      { en: "The page a reel's caption promises used to open with a paragraph. Somebody arriving from Instagram, mid-scroll, on a phone, had to read 250 words before the page proved it knew anything about Denver. The whole season is now an instrument in the first screen — four elevation bands running downhill, each one a link into its own section — and the reading is underneath it, unchanged.", es: "La página que promete el pie de un reel abría con un párrafo. Quien llegaba desde Instagram, a mitad de scroll y en el móvil, tenía que leer 250 palabras antes de que la página demostrara saber algo de Denver. Ahora la temporada entera es un instrumento en la primera pantalla —cuatro franjas de altitud que bajan, cada una enlazada a su sección— y la lectura va debajo, igual que antes." },
      { en: "Five of the twelve places now carry a photograph OF THAT PLACE, taken in autumn, credited under the image with its author, its licence and a link to the original. The other seven carry none, and that is deliberate: a stock photo of generic aspens under the words \"Kenosha Pass\" is a false statement about a real place on a page advertising a licensed brokerage. The files are served uncropped and framed in CSS, so no saved crop inherits a share-alike clause.", es: "Cinco de los doce sitios llevan ya una fotografía DE ESE SITIO, tomada en otoño, con su autor, su licencia y el enlace al original bajo la imagen. Los otros siete no llevan ninguna, y es a propósito: una foto de banco de álamos genéricos bajo las palabras «Kenosha Pass» es una afirmación falsa sobre un sitio real, en una página que anuncia una correduría con licencia. Los ficheros se sirven sin recortar y se encuadran por CSS, para que ningún recorte guardado herede la cláusula de «compartir igual»." },
      { en: "The guide is signed. A stranger who has just read two thousand words of local knowledge sees who wrote them before being asked anything — and, like the footer's brokerage line, the signature renders only where the operator has actually named the advisors.", es: "La guía va firmada. Quien acaba de leer dos mil palabras de conocimiento local ve quién las escribió antes de que se le pida nada — y, como la línea de la correduría en el pie, la firma solo aparece donde el operador ha nombrado de verdad a los asesores." },
    ],
  },
  {
    version: "0.84.0",
