...
import { Activity, Clock, Flame, Loader2, TrendingUp, Users } from "lucide-react";
...
export function AnalyticsView() {
  ...
  const dayMax = Math.max(1, ...data.leads_per_day.map((d) => d.count));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-white/10 bg-white/[0.02] p-5">
        <h2 className="text-sm font-semibold text-white mb-3">{t("analytics.perDay")}</h2>
        {data.leads_per_day.length === 0 ? (
          <p className="text-sm text-gray-600">{t("analytics.noData")}</p>
        ) : (
          <div className="flex items-end gap-1.5 h-28">
            {data.leads_per_day.map((d) => (
              <div key={d.date} className="flex-1 flex flex-col items-center justify-end gap-1" title={`${d.date}: ${d.count}`}>
                <div
                  className="w-full rounded-t bg-eko-violet/60"
